...
// star logo Windows and Linux use — see build/epona-mac-icon.png. The master is
// a bare squircle filling its whole canvas, so this insets it to Apple's macOS
// grid: the shape occupies 824 of a 1024 canvas (80.5%) and the rest is
...
//
// Requires ImageMagick 7 (`magick`). Nothing in CI runs this — regenerate by
// hand when the master changes:
...
const SOURCE = join(repoRoot, 'build', 'epona-mac-icon.png')
...
try {
  // Epona's master is srgb with NO alpha channel: its rounded corners are
  // flattened to opaque black. Creidhne's is srgba, which is why creidhne's
  // copy of this script has no step like this one. Insetting an opaque master
  // onto the transparent grid below would put a black square behind the
  // squircle in the Dock.
  //
  // Flood-filling the four corners recovers the shape. It is safe because the
  // corners are pure black while the squircle body is srgb(3,15,39) navy —
  // about 9.5% away, so nothing inside the artwork is reachable from a corner.
  // Measured against Creidhne.png, whose alpha is genuine: this leaves 2.44%
  // of the canvas transparent where creidhne's master has 2.40%, and the
  // result barely moves between 1% and 8% fuzz. The hard alpha edge it
  // produces is resampled back to a soft one by the downscale that follows.
  //
  // Idempotent: if the master is ever re-exported with real alpha, the corners
  // are already transparent and this becomes a no-op.
  const [width, height] = execFileSync('magick', ['identify', '-format', '%w %h', SOURCE])
    .toString()
    .split(' ')
    .map(Number)
  const corners = [
  ...
  const opaque = join(work, 'alpha.png')
  magick([
    SOURCE,
    '-alpha',
    'set',
    ...
    '-fill',
    'none',
    ...corners.flatMap(([x, y]) => ['-draw', `alpha ${x},${y} floodfill`]),
    opaque
  ])
